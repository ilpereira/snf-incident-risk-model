...
30 trials of Bayesian optimization (TPE sampler) per target, searching over learning rate, max depth, min child weight, subsample, colsample_bytree, and L1/L2 regularization. Objective: mean PR-AUC across 5 facility-held-out CV folds.
...
**Three separate models vs. multi-label.** Separate models per target are easier to calibrate, explain, and update independently. Multi-task learning becomes attractive at >100K residents where joint training provides meaningful regularization.
...
- **RTH** calibration is good within its compressed range (0–0.10): predicted probabilities track observed rates, but all predictions remain below 0.15, which reflects the score discretization rather than miscalibration.
...
- RTH produces only 5 distinct decile groups rather than 10, caused by score discretization collapsing many residents into the same predicted values.
...
![](outputs/plots/shap_importance_fall_30d.png)
...
![](outputs/plots/shap_dependence_fall_30d.png)
...
![](outputs/plots/shap_summary_fall_30d.png)
...
![](outputs/plots/shap_dependence_rth_60d.png)
...
![](outputs/plots/shap_importance_rth_60d.png)
...
![](outputs/plots/shap_importance_wound_60d.png)
...
![](outputs/plots/shap_dependence_wound_60d.png)
...
![](outputs/plots/shap_summary_wound_60d.png)
...
- `prior_days_since_last_fall` cross-contaminates into wound (normalized importance ~0.6) and RTH (~0.1), suggesting a recent fall is a broader signal of acute health deterioration, not just a predictor of subsequent falls.
- `dx_dementia` is nearly exclusively a wound feature (normalized ~1.0 for wound vs ~0.1 for fall and near-zero for RTH), validating that the three models have learned meaningfully distinct clinical signatures rather than duplicating the same signal.
